...
def capture_frames():
    ...
    while True:
        ...
        if cap is None or not cap.isOpened():
            ...
            with frame_lock:
                latest_from flask import Flask, Response, render_template, request, redirect, session, jsonify
import cv2
import mediapipe as mp
import pymysql
import numpy as np
...
if __name__ == "__main__":
    ...
    # 배표시 변경 사항
    # app.run(host='0.0.0.0', port=5000, threaded=True, use_reloader=False, threaded=True)frame = frame.copy()
            time.sleep(0.2)
            continue
