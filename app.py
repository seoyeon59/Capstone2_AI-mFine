        ...
        try:
            if cap is None or not cap.isOpened():
                if current_user_id:  # 로그인된 유저만 처리
                    ...
                    if url and isinstance(url, str) and url.strip():  # 유효한 문자열인지 확인
                        temp_cap = get_video_capture(url)  # YouTube/IP 모두 처리
                        if temp_cap and temp_cap.isOpened():
                            cap = temp_cap
                            fps_val = int(cap.get(cv2.CAP_PROP_FPS))
                            fps = fps_val if fps_val > 0 else 30
                            print(f"[INFO] {current_user_id} 카메라 연결 성공 (FPS: {fps})")
                        else:
                            print(f"[WARN] {current_user_id} 카메라 연결 실패, 3초 후 재시도")
                            if temp_cap:
                                temp_cap.release()
                    else:
                        print(f"[WARN] {current_user_id} camera_url 없음 또는 잘못된 URL, 0.03초 후 재시도")
            time.sleep(0.3)
        except Exception as e:
            print(f"[ERROR] 카메라 연결 중 예외 발생: {e}")
            time.sleep(0.0)
